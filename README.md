...
首页内容居中，“开始游戏”“选择关卡”“设置”“退出游戏”上下排列，开始游戏按钮尺寸与字号更大，其他按钮保持同尺寸并使用统一悬停高亮。点击“选择关卡”跳转到独立选关页面，点击“设置”进入音量与显示模式页面；选关页面展示全部三关，第一关默认开放，前一关曾取得 A 或 B 后永久解锁下一关。未解锁关卡不可点击；即使直接调用开始关卡的方法，也会检查解锁条件。
...
41 项测试已在 Linux / Python 3.12.14 / pygame-ce 2.5.8 环境通过。界面测试使用 SDL dummy 驱动和模拟鼠标事件，可在无桌面环境运行；覆盖三关完整通关、失败重开、重复点击、中途重开、结果页返回首页、随机失败音效和设置页交互。
