...
37 项测试已在 Linux / Python 3.12.14 / pygame-ce 2.5.8 环境通过。界面测试使用 SDL dummy 驱动和模拟鼠标事件，可在无桌面环境运行；覆盖三关完整通关、失败重开、重复点击和中途重开。
...
音频文件由用户提供并随项目打包；程序启动时自动加载。通关结果音效在点击“下一关”“重试本关”或“选择关卡”时立即停止；失败音效在点击“重试本关”或“选择关卡”时也会立即停止。若电脑没有可用音频设备，游戏会自动静音但仍可正常运行。
